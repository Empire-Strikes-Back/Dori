  ...
  (go
    (let [complete| (chan 1)]
      ...
      (let [john-connor (<p! (.create John-Connor
                                      (clj->js
                                       {:repo (.join path (.homedir os) ".Dori" "John-Connor")})))
            beeblebrox (<p!
                        (->
                         (.createInstance
                          Beeblebrox john-connor
                          (clj->js
                           {"directory" (.join path (.homedir os) ".Dori" "Beeblebrox")}))
                         (.catch (fn [ex]
                                   (println ex)))))]
        (println (.. beeblebrox -identity -id)))

      (let [socket (WebSocket. "ws://localhost:3355/corn")]
        (.on socket "open" (fn []

...
(defn column-names
  [conn]
  (->>
   (Dori.query/q {:q :all-attributes :conn conn})
   (sort)
   (into []
         (comp
...
(defn process
  ...
  (go
    (let [ops| (chan 10)
          ...
          schema-databases (read-string (->> (.join path js/__dirname "src/Dori/schema.edn")
                                             (.readFileSync fs)
                                             (.toString)))]
      (let []
        (<! (Arthur-Dent.api/transact conn-databases schema-databases))
        (->>
         (<! (Dori.query/q {:q :all-attributes :conn conn-databases}))
         (sort)
         (println))))))